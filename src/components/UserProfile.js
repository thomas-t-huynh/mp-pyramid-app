...
function UserProfile({ userData }) {
    ...
    const getNearbyRoutes = (e) => {
        ...
        axios
            .get(`https://maps.googleapis.com/maps/api/geocode/json?address=${addressQuery}&key=AIzaSyCVeIhK0QtZ--Y6VF8XmnSrLfVzPqo4nZs`)
            .then((res) => {
                const coordinates = res.data.results[0].geometry.location
                console.log(res.data.results[0])
                axios
                    .get(`https://www.mountainproject.com/data/get-routes-for-lat-lon?lat=${coordinates.lat}&lon=${coordinates.lng}&maxDistance=30&key=200690742-42241ba1e91a3117df55a44758abbb73`)
                    .then((res) => {
    ...
    return (
        <div>
            <form onSubmit={e => getNearbyRoutes(e)}>
                ...
                {error && <h3>{error}</h3>}
                <input type="text" placeholder="Enter an address" onChange={e => setAddress(e.target.value)} value={address} />
                <button>Go!</button>
            </form>
            <div>
